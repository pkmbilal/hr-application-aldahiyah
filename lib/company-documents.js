...
export async function listCompanyDocuments() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, title, category, description, storage_path, file_name, file_type, file_size, share_token, is_active, created_at, updated_at"
    )
...
function isCompanyDocumentsSetupError(error) {
  return error?.code === "42P01" || error?.message?.includes("company_documents");
}
