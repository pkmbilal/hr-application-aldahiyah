...
export async function createEmployeeSiteAllowanceNotification({ profile, employeeId, allowanceId, title, body, href }) {
  if (profile?.role !== "admin") {
    return;
  }
  ...
  const { error } = await supabase.rpc("create_employee_notification", {
    ...
    p_actor_user_id: profile.id,
    p_event_type: "created",
    p_entity_type: "site_allowance",
    p_entity_id: allowanceId,
    p_title: title,
    p_body: body,
    p_href: href,
  });
  ...
  if (error) {
    console.error("Failed to create employee site allowance notification:", error.message);
  }
}
