...
export const EMPLOYEE_BUCKET = "employee-documents";
...
export async function withSignedEmployeeUrls(employee) {
  ...
  for (const key of ["passport_copy_path", "iqama_copy_path", "license_upload_path"]) {
    signed[`${key.replace("_path", "")}_url`] = await createSignedUrl(supabase, employee[key]);
  }

  signed.employee_other_ids = await Promise.all(
    (employee.employee_other_ids || []).map(async (item) => ({
      ...item,
      file_url: await createSignedUrl(supabase, item.file_path),
    }))
  );
...
async function createSignedUrl(supabase, path) {
  ...
  const { data } = await supabase.storage.from(EMPLOYEE_BUCKET).createSignedUrl(path, 60 * 10);
